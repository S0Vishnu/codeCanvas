...
import { useObstacleHitHandlers } from "./hooks/useObstacleHitHandlers";
...
export function GameScene({
    ...
    const { handleObstacleHit } = useObstacleHitHandlers(
        setCoins,
        setPencilScale,
        EndGame
    );
    ...
    const spawnConfig = {
        ...
        lead: { weight: 0.3, minDistance: 8 }, // 30% chance
    ...
    useFrame((_, delta) => {
        ...
        if (spawnTimer.current > spawnInterval.current) {
            ...
            const safeZ =
                lastSpawn?.z != null
                    ? Math.min(lastSpawn.z - minDist, -10)
                    : -10;
        ...
        obstacles.current.forEach((ob) => {
            ...
            const dist = playerPos.current.distanceTo(
                new Vector3(ob.x, ob.y, ob.z)
            );

            if (dist < 0.4) {
                handleObstacleHit(ob.kind);
                ob.z = 1000; // move it out of view / deactivate
            }
        });
        ...
        if (left)
            playerPos.current.x = Math.max(
                -2,
                playerPos.current.x - moveSpeed * delta
            );
        if (right)
            playerPos.current.x = Math.min(
                2,
                playerPos.current.x + moveSpeed * delta
            );
    ...
    return (
        <>
            ...
            <group
                position={[playerPos.current.x, playerPos.current.y + 0.3, 0]}
            >
                <RigidBody
                    colliders="cuboid"
                    restitution={0.2}
